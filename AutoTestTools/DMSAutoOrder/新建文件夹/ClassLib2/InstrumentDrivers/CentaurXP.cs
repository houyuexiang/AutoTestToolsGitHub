    ...
    class CentaurXP: InstrumentBase
    {
        ...
        private void GetOrder()
        {
            ...
            while (startpos >= 0 && endpos > 0)
            {
                ...
                foreach (string S in ASTMString)
                {
                    ...
                    string flag = tmp.Substring(1, 1);
                    string[] splits;
                    string ordertest;
                    splits = tmp.Split(Convert.ToChar("|"));
                    if (splits[0].Length > 1)
                    {
                        flag = splits[0].Substring(splits[0].Length - 1, 1);
                    }
                    if (flag == "O")
                    {
                        Sampleno = splits[2];
        ...
        private string result(string sampleid,List<string> list)
        {
            ...
            foreach (string s in list)
            {
                ...
                Oitem += "\\^^^" + temp;
                int seq = seeds + 1;
                resultstring += "[R|" + seq++ + "|^^^" + temp + "^1^^^DOSE|" + MakeRandomValue(seeds + DateTime.Now.Millisecond + DateTime.Now.Second) + "|ng/mL||||F||||" + datetime + GlobalValue.CR + "]";
                seeds++;
            }
            if (Oitem == "") return "";
            Oitem = Oitem.Substring(1);
            Send = "[H|\\^&|||NG_LIS|||||LIS_ID||P|1" + GlobalValue.CR + "]";
            Send += "[P|1|289463|||JiangZhuZhi||19360418|F|||||||||||||||||26" + GlobalValue.CR + "]";
            Send += "[O|1|" + sampleid + "||" + Oitem + "|R||||||||||||||||||||F" + GlobalValue.CR + "]";
            Send += resultstring;
            Send += "[L|1" + GlobalValue.CR + "]";
            return Send;
        }
